...
用户输入账号密码
   ↓
前端 Login.vue 表单校验 → store.login() → POST /api/user/login
   ↓
...
| 模块 | 已实现能力 |
|------|-----------|
| 用户与认证 | 注册、登录（BCrypt + 历史 MD5 自动升级）、登出（Token 黑名单）、图形验证码（Hutool，4 位，5 分钟）、GitHub / QQ 第三方登录、修改密码、重置密码、账号注销、封禁拦截 |
