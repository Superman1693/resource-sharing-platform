-- ============================================================================
--  编程知识星球（笔记资源分享平台）—— 数据库完整脚本
--  ============================================================================
--  【本文件是数据库的「人类可读全量快照」，供查阅与手工建库使用】
--
--  与 Flyway 的关系（重要）：
--    · 结构变更的**机械权威**是 Flyway 迁移脚本，位于
--        user-center/src/main/resources/db/migration/
--          V1__baseline_schema.sql                   —— 基线（25 张表）
--          V2__multi_tenant_scope_and_error_log.sql  —— 多租户作用域 + 错误日志
--    · 本文件 = V1 + V2 + … 的最新全量结果（26 张表）；
--    · 两者必须保持同步：新增迁移脚本时，请同步更新本文件。
--
--  适用环境：MySQL 8.0+（兼容 MySQL 5.7 / MariaDB 10.5+）
--  数据库名：yiya
--
--  ── 前置条件 ────────────────────────────────────────────────────────────
--  ⚠️ MySQL 必须为 8.0+：Flyway 9.x 社区版不支持 MySQL 5.7，
--     版本过低会在启动阶段抛 FlywayEditionUpgradeRequiredException。
--
--  ── 使用方式 ────────────────────────────────────────────────────────────
--  ① 全新数据库（最推荐）：什么都不用做，直接启动后端
--       cd user-center && ./mvnw spring-boot:run
--     Flyway 会依次执行 V1 → V2，自动建出与本文档等价的完整结构。
--
--  ② 全新数据库（手工建库）：
--       mysql -u root -p < db/schema.sql
--     ⚠️ 此后若再启动应用，请先把 spring.flyway.baseline-version 设为 2，
--        否则 Flyway 会从 V1 开始判定并重复执行 V2（报「字段已存在」）。
--
--  ③ 已有数据的旧库（结构等价于 V1）：
--     直接启动后端即可 —— Flyway 检测到非空 schema，会标记基线为 1，
--     只执行 V2（新增 current_star_id / comment.star_id / error_log）。
--     也可手工执行本文件第 13 部分的增量语句。
--
--  ── 目录 ────────────────────────────────────────────────────────────────
--    1. 初始化
--    2. 用户体系      user · user_follow
--    3. 星球体系      star · star_member                （多租户隔离单元）
--    4. 内容体系      note · note_column · comment · tag · note_tag
--    5. 资源体系      resource
--    6. 互动体系      like_record · view_record · note_collection · report
--    7. 私信与通知    message_conversation · message · notification
--    8. 知识图谱      knowledge_map · knowledge_node
--    9. 学习路径      learning_path · learning_path_node
--   10. 积分体系      points_account · points_log · sign_in_record
--   11. 后台管理      admin_log · error_log
--   12. 增量升级脚本（仅供已有数据库使用）
--
--  ── 公共约定 ────────────────────────────────────────────────────────────
--    · 字符集 utf8mb4 / 排序 utf8mb4_unicode_ci
--    · 主键 BIGINT 自增；命名一律 snake_case
--    · 逻辑删除字段 is_delete（0=未删除，1=已删除），由 MyBatis-Plus 自动过滤
--    · 多租户列 star_id 存在于 6 张表：note / comment / star / star_member /
--      knowledge_map / note_column（user 表没有 star_id，用户的星球归属记录在
--      star_member 中，且一人可加入多个星球）
--    · 内容隔离采用「请求级星球作用域 + 表白名单」：
--      前端进入星球页面时携带请求头 X-Star-Id，AuthInterceptor 校验成员身份后
--      写入 UserContext，TenantInterceptor 才对
--      note / comment / note_column / knowledge_map 注入 star_id 条件；
--      未携带时不做收窄，保证首页、搜索、热榜等跨星球公开读取不受影响
-- ============================================================================
...
CREATE DATABASE IF NOT EXISTS `yiya`
...
CREATE TABLE `user` (
  ...
  `current_star_id` BIGINT     DEFAULT NULL COMMENT '当前所在星球ID（多租户上下文，为空时回退为最早加入的星球）',
...
-- ---------------------------------------------------------------------------
DROP TABLE IF EXISTS `note_column`;
...
CREATE TABLE `comment` (
  ...
  `star_id`     BIGINT       DEFAULT NULL COMMENT '所属星球ID（由所属笔记派生，多租户列）',
...
-- ---------------------------------------------------------------------------
-- 11.2 前端错误日志表 error_log          （全局表，不参与星球租户隔离）
--      承载前端 errorTracker.js 批量上报的 JS 错误 / Promise 异常 / Vue 组件异常，
--      提供管理端分页查询入口（GET /api/error/list），形成可回溯的闭环。
-- ---------------------------------------------------------------------------
...
--  ----------------------------------------------------------------------------
--  ① 推荐方式：交给 Flyway 自动执行
--       cd user-center && ./mvnw spring-boot:run        # 启动即自动迁移
--     配置见 application.yml 的 spring.flyway.*；
--     已有库首次接入时会被标记为基线版本 1，只执行 V2 及之后的脚本。
--
--  ② 手工方式：若暂时不想引入 Flyway，可按下面的语句手工升级。
--     [注意] MySQL 的 ALTER TABLE 不支持 IF NOT EXISTS（那是 MariaDB 语法），
--            重复执行会报「字段已存在」，属正常现象。
-- ============================================================================
...
UPDATE `comment` c
  ...
  WHERE c.`star_id` IS NULL AND n.`star_id` IS NOT NULL;

-- 3) 新增 error_log 表（完整 DDL 见第 11.2 节，此处略）

-- 4) 第三方登录账号的空密码改为明确的占位符（无法被密码登录命中）
UPDATE `user`
  SET `user_password` = '!oauth-no-password!'
  WHERE `user_password` = '' OR `user_password` IS NULL;

-- ---------------------------------------------------------------------------
-- 更早的历史迁移（原 V2~V9）
-- ---------------------------------------------------------------------------
-- 关注/私信/学习路径、星球公告、资源上传者、笔记收藏、标签体系、
-- 积分体系、举报记录、专栏/合集、索引优化
-- —— 这些内容已全部包含在上方第 2~11 节的全量建表语句中，
--    对于全新库无需额外执行；对于老库，它们的表与字段应已存在。

-- ---------------------------------------------------------------------------
-- 其他说明
-- ---------------------------------------------------------------------------
-- 1. 标签数据回填（把 note.tags JSON 迁到 note_tag 关联表）需调用后端接口：
--      curl -X POST http://localhost:8080/api/tag/migrate -H "Authorization: Bearer <管理员token>"
-- 2. Redis 无需迁移（缓存/计数/锁均会自动重建）；
--    Elasticsearch 建议在升级后触发一次全量索引同步。
-- ============================================================================
